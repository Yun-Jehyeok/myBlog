...
router.post("/login", (req, res) => {
  ...
  if (!email) return res.status(400).json({ msg: "이메일을 작성해주세요." });else if (!password) return res.status(400).json({ msg: "비밀번호를 작성해주세요." });

  User.findOne({ email }).then(user => {
    if (!user) return res.status(400).json({ msg: "이메일을 확인해주세요." });

    bcrypt.compare(password, user.password).then(isMatch => {
      if (!isMatch) return res.status(400).json({ msg: "비밀번호를 확인해주세요." });
